...
class AutoencoderKL(pl.LightningModule):
    def __init__(self,
        ...
        super().__init__()
        if int(pl.__version__.split(".", 1)[0]) >= 2:
    ...
    def training_step(self, batch, batch_idx, *optimizer_idx):
        if optimizer_idx:
            return self._training_step(batch, batch_idx, optimizer_idx=optimizer_idx[0])

        if not self.automatic_optimization:
            opt_ae, opt_disc = self.optimizers()

            opt_ae.zero_grad()
            aeloss = self._training_step(batch, batch_idx, optimizer_idx=0)
            self.manual_backward(aeloss)
            opt_ae.step()

            opt_disc.zero_grad()
            discloss = self._training_step(batch, batch_idx, optimizer_idx=1)
            self.manual_backward(discloss)
            opt_disc.step()

            return {"loss": (aeloss + discloss).detach()}

        return self._training_step(batch, batch_idx, optimizer_idx=0)
