...
class ReconstructorLegacy(nn.Module):
...
class Reconstructor(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        if self.reconstructor_type == 'LeNet':
            features = self.feature_extractor(torch.cat([x1, x2], dim=1))
            features = features.mean(dim=[-1, -2]).reshape(x1.shape[0], -1)
            return self.path_indices(features).reshape(x1.shape[0], -1), None
        elif self.reconstructor_type == 'ResNet':
            self.features_extractor(torch.cat([x1, x2], dim=1))
            features = self.features.output.reshape([x1.shape[0], -1])
            return self.path_indices(features).reshape(x1.shape[0], -1), None
