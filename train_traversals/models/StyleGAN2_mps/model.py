...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, update_emas=False, skip_torgb=False, **layer_kwargs):
        ...
        # ToRGB is unnecessary for callers that only consume intermediate
        # synthesis features. Skipping it also avoids carrying the RGB skip
        # pyramid through every prefix block.
        if not skip_torgb:
            if img is not None:
