...
from lib.aux import update_progress, update_stdout
...
def compute_identity_scores(path_images, id_comparator, two_sided=False):
    """Compare every frame with frame 0, or with the midpoint for legacy two-sided traversals."""
    num_frames = path_images.size(0)
    if num_frames == 0:
        return []
    if two_sided and num_frames % 2 == 0:
        raise ValueError("Two-sided traversals must have an odd number of frames so the midpoint is unambiguous")

    reference_index = num_frames // 2 if two_sided else 0
    reference = path_images[reference_index:reference_index + 1].div(255.0).mul(2.0).add(-1.0)
    scores = []
    with torch.no_grad():
        for frame_index in range(num_frames):
            frame = path_images[frame_index:frame_index + 1].div(255.0).mul(2.0).add(-1.0)
            scores.append(id_comparator(reference, frame).item())
    return scores
...
def main():
    ...
    Options:
        ...
        --two-sided   : use the midpoint as the original frame for legacy negative-to-positive traversals. By default,
                        traversals are one-sided and frame 0 is the original.
        ================================================================================================================
    ...
    parser.add_argument('--two-sided', action='store_true',
                        help="use the midpoint as the original frame for legacy two-sided traversals")
    ...
    sample_dirs = sorted(
        ...
        and osp.isfile(osp.join(latent_traversal_dir, dirname, 'paths_latent_codes.pt'))
        and osp.isdir(osp.join(latent_traversal_dir, dirname, 'paths_images'))
    )
    if not sample_dirs:
        raise FileNotFoundError("No sample traversal directories found under {}".format(latent_traversal_dir))
    ...
    ##                                                                                                                ##
    ####################################################################################################################
    traversal_roots = [(latent_traversal_dir, sample_dirs)]
    for hashes_dir, hashes in traversal_roots:
        # For each sample and all of its traversal paths, calculate attribute predictions across every image sequence.
        hash_cnt = 0
        for h in hashes:
            hash_cnt += 1
            if args.verbose:
            ...
            if args.two_sided and num_of_img_per_path % 2 == 0:
                raise ValueError(
                    "Two-sided traversal {} must have an odd number of frames; found {}".format(
                        h, num_of_img_per_path
                    )
                )
